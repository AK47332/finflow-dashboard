...
export default function IncomePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return incomes
      ...
      .filter((i) =>
        q
          ? [i.title, i.client, i.category, i.paymentMethod]
              .filter(Boolean)
  ...
  }, [incomes, range, category, search, sortDesc]);
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Income</h1>
          <p className="mt-1 text-sm text-muted-foreground">Track every dollar coming in.</p>
        </div>
        <Button onClick={openAdd} className="bg-gradient-income text-income-foreground hover:opacity-90">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="ft-card p-5">
          <div className="flex items-center gap-3">
            <div className="ft-stat-icon bg-income-soft text-income">
      ...
      <div className="ft-card flex flex-col gap-3 p-4">
          ...
          {RANGE_TABS.map((t) => (
            <button
              ...
              className={cn(
                ...
                range === t
                  ? "bg-gradient-income text-income-foreground shadow-soft"
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative min-w-[200px] flex-1">
            ...
            <Input
              placeholder="Search title, client, method…"
              value={search}
          ...
          <Button
            variant="outline"
            onClick={() => exportIncomesCSV(filtered)}
          ...
          <Button
            variant="outline"
            onClick={() => exportIncomesPDF(filtered)}
            disabled={!filtered.length}
          >
              ...
              filtered.map((i, idx) => (
                <TableRow key={i.id} className={idx % 2 === 1 ? "bg-muted/20" : undefined}>
                  ...
                  <TableCell className="font-medium text-foreground">
                    {i.title}
                    {i.isPartial && (
                      <Badge variant="outline" className="ml-2 border-profit/30 bg-profit-soft text-profit">
                        Partial
                      </Badge>
                    )}
                  </TableCell>
